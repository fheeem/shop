package com.macademy.shop.cart.controller;
...
public class CartController {
    ...
    public ResponseEntity updateCartItem(@PathVariable("cartItemId")Long cartItemId
        ...
        if(count <= 0){
            return new ResponseEntity("최소1개 이상 담아줘", HttpStatus.BAD_REQUEST);
        } else if(!cartService.validateCartItem(cartItemId, principal.getName())){
            return new ResponseEntity("수정권한이 없다", HttpStatus.FORBIDDEN);
        }
    ...
    public ResponseEntity orderCartItem(@RequestBody CartOrderDto cartOrderDto
        ...
        for (CartOrderDto cartOrder : cartOrderDtoList) {
            if(!cartService.validateCartItem(
                       cartOrder.getCartItemId(), principal.getClass())) {
